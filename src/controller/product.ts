...
export default class ProductController {
    ...
    static async add(ctx: Context, next: NextFunction) {
        ...
        try {
            let a: Product.ProductInterface = {
                tag: 'tag',
                title: title, 
                content: content,
                author: 'au',
            ...
            console.log(a)
            let note = await ProductModel.create(a)
            console.log(note)
            ctx.status = 200;
            ctx.body = note;
            console.log('存库成功')
    ...
    static async get(ctx: Context) {
        ...
        try {
            if (id) {
                // 有id获取单条
                note = await ProductModel.findById(id).exec()
            } else {
                // 没有id获取全部
                note = await ProductModel.find().exec()
            }
    ...
    static async update(ctx: Context, next: NextFunction) {
        ...
        try {
            note = await ProductModel.findByIdAndUpdate(id, {
                title: 'he',
                content: content
            }, {
                new: true
